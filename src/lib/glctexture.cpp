...
//-----------------------------------------------------------
void glcTexture::SetEnvColor(float *color) {
    env_color[0] = color[0];
    env_color[1] = color[1];
    env_color[2] = color[2];
    env_color[3] = color[3];
...
void glcTexture::CreateTextureFromPNG(std::string nome, int id) {
    ...
    // get info about png
    png_get_IHDR(png_ptr, info_ptr, (png_uint_32 *) &width, (png_uint_32 *) &height, &bit_depth, &color_type, NULL,
                 NULL, NULL);
...
/* TODO: Preparar a criação de texturas por imagens JPG através da biblioteca "libjpeg-dev"
  // Include necessário:
  //   #include "jpeglib.h"
  // Lib necessária para incluir no projeto:
  //   libjpeg
